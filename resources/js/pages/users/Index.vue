...
type UserType = { id: string; name: string; portal: string };
type IdentityUser = {
    id: string;
    name: string;
    email: string;
...
const selectedTypeId = ref('arbo');

const typeById = (id: string) => props.userTypes.find((t) => t.id === id);
</script>
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4">
        <div class="flex items-center justify-between">
            <Heading title="Users" description="Manage all users across every portal." />
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b bg-muted/50">
                        ...
                        <th class="px-4 py-3 text-left font-medium">Portal</th>
                ...
                <tbody>
                    <tr v-for="user in users" :key="user.id" class="border-b last:border-0 hover:bg-muted/30 cursor-pointer" @click="editingUser = user">
                        ...
                        <td class="px-4 py-3 text-muted-foreground">{{ typeById(user.user_type_id)?.name ?? user.user_type_id }}</td>
                        <td class="px-4 py-3 text-muted-foreground">{{ typeById(user.user_type_id)?.portal ?? '—' }}</td>
                    ...
                    <tr v-if="users.length === 0">
                        <td colspan="6" class="px-4 py-6 text-center text-muted-foreground">No users yet.</td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            >
                ...
                <div class="grid gap-2">
                    ...
                    <select
                        ...
                        @change="selectedTypeId = ($event.target as HTMLSelectElement).value"
                    >
